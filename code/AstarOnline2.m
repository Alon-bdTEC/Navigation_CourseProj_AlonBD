function [path,C,List_FromCurrToNewBest] = AstarOnline2(graph, vertss, S, T)
    ...
    while not(isempty(O)) && not(targetClosed)
        ...
        elseif isempty(C) || back_points_Prev == 0 
            ...
            % From S to x_bestNew (without S)
            forward_pt_S_to_XbestNew = findBackPointer(C,xbestNew);
            forward_pt_S_to_XbestNew = forward_pt_S_to_XbestNew(2:end);

            for w = 1:size(back_pt_S_to_XbestPrev,1)
                indexShortcut =...
                    find(forward_pt_S_to_XbestNew == back_pt_S_to_XbestPrev(w));
                if ~isempty(indexShortcut)
                    back_pt_S_to_XbestPrev = back_pt_S_to_XbestPrev(1:w);
                    forward_pt_S_to_XbestNew = forward_pt_S_to_XbestNew(indexShortcut:end);
                    break;
                end
            end
            
            total_trav = [back_pt_S_to_XbestPrev;forward_pt_S_to_XbestNew];
